...
from .models import (
    ...
    MusicTrack, MealTemplate, WorkoutTemplate, Feedback
)
from .utils import (
    clamp01, mean_std, get_today_iso, cosine_pref_fit, novelty_bonus,
    risk_penalties_meal, risk_penalties_workout, zone_from_state,
    energy_cap_from_state, target_bpm_from_state
)
...
def compute_state_entries(user: UserProfile,
                          ...
                          activity_entries: List[ActivityEntry]) -> Dict[str,int]:
...
def _calculate_recovery_factor(user_id: str) -> float:
    ...
    if steps_std > 0:
        ...
        recovery_factor = clamp01(1 - strain_z_score / 2.0)  # Higher strain = lower recovery
    else:
...
def filter_music_candidates(user: UserProfile, state: Dict[str,int], arm_id: str) -> List[MusicTrack]:
    ...
    pool = [m for m in MUSIC if m.energy <= min(cap, ARMS["music"][arm_id]["energy_cap"]) and
            any(g in m.genres for g in ARMS["music"][arm_id]["genres"]) and
            abs(m.bpm - bpm_tgt) <= 15]
    if not pool:
        pool = [m for m in MUSIC if m.energy <= cap and abs(m.bpm - bpm_tgt) <= 20]
    return pool or MUSIC  # fallback


def filter_meal_candidates(user: UserProfile, state: Dict[str,int], arm_id: str) -> List[MealTemplate]:
...
def filter_workout_candidates(user: UserProfile, state: Dict[str,int], arm_id: str) -> List[WorkoutTemplate]:
    ...
    if not allowed:
        ...
        allowed = [w for w in WORKOUTS if ({"Z2_low":0,"Z2":1,"Tempo":2}[w.intensity_zone] <= {"Z2_low":0,"Z2":1,"Tempo":2}[cap_zone])]
    return allowed


def rank_music(cands: List[MusicTrack], user: UserProfile, state: Dict[str,int]) -> List[Tuple[MusicTrack, float]]:
    ...
    for m in cands:
        ...
        score = 0.35*GoalFit + 0.30*StateFit + 0.25*PrefFit + 0.10*novelty_bonus(m.id,"music")
        results.append((m, score))
    results.sort(key=lambda x: x[1], reverse=True)
    return results


def rank_meals(cands: List[MealTemplate], user: UserProfile, state: Dict[str,int]) -> List[Tuple[MealTemplate,float]]:
    """Rank meal candidates by nutritional needs and preferences."""
    # Compute macro gaps from today's totals
    today = get_today_iso(None)
    todays = next((n for n in reversed(NUTRITION.get(user.user_id, [])) if n.date <= today), None)
    P_now, fiber_now, sugar_now, sodium_now = (todays.protein_g, todays.fiber_g, todays.sugar_g, todays.sodium_mg) if todays else (0,0,0,0)
    ...
    for meal in cands:
        ...
        protein_fill = min(1.0, meal.protein_g / (gaps["protein"]+1e-6)) if gaps["protein"]>0 else 0.5
        fiber_fill = min(1.0, meal.fiber_g / (gaps["fiber"]+1e-6)) if gaps["fiber"]>0 else 0.4
        sugar_ok = 1.0 if meal.sugar_g <= gaps["sugar_room"] else 0.2
        sodium_ok = 1.0 if meal.sodium_mg <= gaps["sodium_room"] else 0.3
        GoalFit = clamp01(0.6*protein_fill + 0.2*fiber_fill + 0.1*sugar_ok + 0.1*sodium_ok)
        StateFit = 1.0 if state["Fuel"] < 60 else 0.7
        score = 0.35*GoalFit + 0.30*StateFit + 0.25*PrefFit + 0.10*novelty_bonus(meal.id,"meal")
        results.append((meal, score))
    results.sort(key=lambda x: x[1], reverse=True)
    return results


def rank_workouts(cands: List[WorkoutTemplate], user: UserProfile, state: Dict[str,int]) -> List[Tuple[WorkoutTemplate,float]]:
    ...
    for w in cands:
        ...
        GoalFit = 1.0 if ("endurance" in w.focus_tags and user.goals=="endurance") else 0.7
        score = 0.35*GoalFit + 0.30*StateFit + 0.25*PrefFit + 0.10*novelty_bonus(w.id,"workout") - 0.25*Risk
        results.append((w, score))
    results.sort(key=lambda x: x[1], reverse=True)
    return results


def choose_domain(intent: Optional[str], state: Dict[str,int], hours_since_last_meal: Optional[float]) -> str:
