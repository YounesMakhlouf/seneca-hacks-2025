...
from .models import (
    SleepEntry, NutritionEntry, ActivityEntry, UserProfile,
    MusicTrack, MealTemplate, WorkoutTemplate,
    RecommendRequest, RecommendResponse, Feedback
)
from .data_loader import data_loader
from .db import insert_many, collection_count
import asyncio, os
...
async def _maybe_seed_mongo():
    ...
    if collection_count("users") == 0:
        ...
        if data_loader.load_all_data():
            ...
            sleep_docs = [s.model_dump() for arr in data_loader.sleep.values() for s in arr]
            nutrition_docs = [n.model_dump() for arr in data_loader.nutrition.values() for n in arr]
            activity_docs = [a.model_dump() for arr in data_loader.activity.values() for a in arr]
            measurement_docs = [m for arr in data_loader.measurements.values() for m in arr]
...
# Import endpoints to register them with the app
