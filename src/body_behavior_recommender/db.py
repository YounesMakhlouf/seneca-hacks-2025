...
MONGODB_URI = os.getenv("MONGODB_URI", "mongodb://bbr:bbrpass@mongo:27017/?authSource=admin&directConnection=true")
...
def get_user(user_id: str) -> Optional[Dict[str, Any]]:
    ...
    with MongoClientWrapper(UserProfile, COLLECTIONS["users"], MONGO_DB_NAME, MONGODB_URI) as client:
...
def get_recent_entries(collection: str, user_id: str, limit: int = 7) -> List[Dict[str,Any]]:
    """Get recent entries for a user from MongoDB."""
    model = COLLECTION_MODELS.get(collection)
    if not model:
        return []

    with MongoClientWrapper(model, COLLECTIONS[collection], MONGO_DB_NAME, MONGODB_URI) as client:
        # Get recent entries sorted by date (descending first, then reverse for chronological asc)
        docs = client.fetch_documents(limit * 2, {"user_id": user_id})  # Fetch more to sort properly
...
def get_user_measurements(user_id: str, limit: int = 10) -> List[Dict[str,Any]]:
    ...
    with MongoClientWrapper(MeasurementEntry, COLLECTIONS["measurements"], MONGO_DB_NAME, MONGODB_URI) as client:
