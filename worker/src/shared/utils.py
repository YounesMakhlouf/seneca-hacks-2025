...
def compute_hr_max(user) -> int:
    """Calculate maximum heart rate for a user."""
    return getattr(user, 'hr_max_override', None) or int(208 - 0.7*user.age)


def target_bpm_from_state(user, state: Dict[str,int]) -> int:
...
def risk_penalties_meal(meal, user) -> float:
    """Calculate risk penalties for a meal."""
    # 1.0 if hard violation (block later), smaller otherwise
    allergens = getattr(user, 'allergens', [])
    diet_flags = getattr(user, 'diet_flags', [])
    
    if any(a in allergens for a in meal.allergens):
        return 1.0
    if diet_flags and not any(flag in meal.diet_ok for flag in diet_flags):
        return 1.0
    return 0.0


def risk_penalties_workout(w, user, state: Dict[str,int]) -> float:
